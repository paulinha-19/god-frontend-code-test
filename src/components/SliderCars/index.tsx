...
const SliderCars = ({ cars }: SliderCarProps) => {
  ...
  const showPrevCar = (): void => {
    setCurrentCar((prevCar) => (prevCar === 0 ? cars.length - 1 : prevCar - 1));
  };
  ...
  const isNextDisabled = currentCar + (isMobile ? 2 : 4) >= cars.length;
  ...
  return (
    <section className={`slider md:container md:mx-auto flex flex-wrap w-full`}>
      <div className={`w-full flex justify-center flex-wrap`}>
        {getVisibleCars().map((car, index) => (
          <div
            key={car.id}
            className={`slider-box w-full ${
              isMobile ? "sm:w-1/2 overflow-hidden" : "sm:w-1/4"
            }`}
          >
            <CarsCard car={car} />
          </div>
      ...
      {isMobile ? (
        <div className="dots-indicator mx-auto mt-10 mb-10">
